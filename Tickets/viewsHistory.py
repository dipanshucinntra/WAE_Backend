...
from Tickets.models import Tickets, History, RescueHistory
...
def createRescue(request):
    try:
        ...
        RescueHistory(
            TicketId_id = request.data['TicketId'],
            Status = request.data['Status'], # On the way, Reached, Work in Progress, Rescue Success
            Remarks = request.data['Remarks'], # work what have done
        ).save()
        
        if request.data['Status'] =="Rescue Success":
            Tickets.objects.filter(pk=request.data['TicketId']).update(RescueAt=serverDateTime)
