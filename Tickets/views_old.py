...
def filter_all(request):
    try:
        ...
        BpCardCode = request.data['BpCardCode']
        ticketsObj = Tickets.objects.filter(BpCardCode = BpCardCode).order_by('-id')[startWith:endWith]
        # ticketsJson = TicketsSerializer(ticketsObj, many=True)
        result = showTickets(ticketsObj)
        return Response({"message":"Successfull","status":200 ,"data":result})
    except Exception as e:
    ...
    try:
        ...
        if Tickets.objects.filter(pk = TicketId).exists():
            ...
            if TicketStatus == "Accepted":
                ...
                Tickets.objects.filter(pk = TicketId).update(TicketStatus = TicketStatus, Status = "Assigned")
            ...
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            employeeObj = Employee.objects.get(SalesEmployeeCode = EmployeeId)
            History( TicketId_id = TicketId, Type = "Service", Remarks = f'Ticket {TicketStatus} by {employeeObj.firstName}').save()
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    try:
        ...
        if Tickets.objects.filter(pk = TicketId).exists():
            ...
            if "TicketStartDate" in requestData:
                ...
                History( TicketId_id = TicketId, Type = "Service", Remarks = f'Ticket start by {employeeObj.firstName}').save()
                History( TicketId_id = TicketId, Type = "Service", Remarks = f'Ticket is `In Progress` by {employeeObj.firstName}').save()
                return Response({"message":"Successfull","status":200 ,"data":[]})
                ...
                employeeObj = Employee.objects.get(SalesEmployeeCode = EmployeeId)
                History( TicketId_id = TicketId, Type = "Service", Remarks = f'Ticket end by {employeeObj.firstName}').save()
    ...
    try:
        ...
        if Employee.objects.filter(SalesEmployeeCode = EmployeeId).exists():
            ...
            if 'Status' in requestData:
                ...
                if Status != "":
                    ticketsObj = Tickets.objects.filter(AssignTo__in = empids, Status = Status).order_by('-id')[startWith:endWith]
                else:
                    ticketsObj = Tickets.objects.filter(AssignTo__in = empids).order_by('-id')[startWith:endWith]
                ...
                ticketsObj = Tickets.objects.filter(AssignTo__in = empids, CreateDate__gte = FromDate, CreateDate__lte = ToDate).order_by('-id')[startWith:endWith]
                
            elif 'Priority' in requestData:
                print('--- Priority key ---')
                Priority = requestData['Priority']
                ticketsObj = Tickets.objects.filter(AssignTo__in = empids, Priority = Priority).order_by('-id')[startWith:endWith]
            elif 'Type' in requestData:
                print('--- Type key ---')
                Type = requestData['Type']
                ticketsObj = Tickets.objects.filter(AssignTo__in = empids, Type = Type).order_by('-id')[startWith:endWith]
            else:
...
def auto_ticket(project, BP, cur_stage, next_stage, tkt_type):
...
def filter_bystage(request):
    try:
        ...
        ticketsObj = Tickets.objects.filter(DeliveryID = StageId).order_by('-id').values("id","Title")
