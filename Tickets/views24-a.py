...
def sc_ticket(request, type):
    ...
    print(type['Type'])
    print(request)
    print(request.data['Frequency'])
    print(request.data['MNo'])
    
    if request.data['Frequency'].lower() =="yearly":
         fq = 1
    elif request.data['Frequency'].lower() =="half yearly":
         fq = 2
    elif request.data['Frequency'].lower() =="quarterly":
         fq=4
    elif request.data['Frequency'].lower() =="monthly":
         fq=12
    else:
        fq=1
    ...
    try:
        for i in range(0, fq):
                ...
                # >>>>>>>>>>>>>> assign checklist >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                if DropDown.objects.filter(DropDownName="Checklist", Field1 =  type['Type'], Field2 =  type['SubType']).exists():
                        print('-----check list found----')
                        checklistObj = DropDown.objects.filter(DropDownName="Checklist", Field1 =  type['Type'], Field2 =  type['SubType'])
                        print(checklistObj)
                        if len(checklistObj) != 0:
                            for obj in checklistObj:
                                TicketChecklist(
