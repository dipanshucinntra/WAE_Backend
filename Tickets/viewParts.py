    ...
    # try:
        ...
        if Tickets.objects.filter(pk = TicketId).exists():
            if len(prItems) != 0:
                ...
                # >>>>>>>>> Update PartRequest Status >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                ticketObj.PartRequest=1
                ticketObj.save()
    ...
    # try:
        ...
        if PartRequest.objects.filter(pk = PRID).exists():
            ...
            if Employee.objects.filter(SalesEmployeeCode = ApproverId).exists():
                ...
                #if approverEmpObj.role == 1 or int(approverEmpObj.SalesEmployeeCode) == int(empObj.reportingTo):
                print(approverEmpObj)
                print(approverEmpObj.role.Name)
                if approverEmpObj.role.Name == 'Admin' or int(approverEmpObj.SalesEmployeeCode) == int(empObj.reportingTo):
