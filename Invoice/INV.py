...
if "error" in json.loads(r.text):
        ...
        res = requests.get(settings.BASEURL+'/Invoices/$count', cookies=r.cookies, verify=False)
        serInvoicesCount = int(res.text)
        count = math.ceil(int(serInvoicesCount)/20)
        print(count)


        mycursor.execute("select * from Invoice_invoice")
        mycursor.fetchall()
        localInvoicesCount = mycursor.rowcount

        print(f'local Industires count: {localInvoicesCount} and server Industires count {serInvoicesCount}')
        if localInvoicesCount < serInvoicesCount:
            skip=0
            for i in range(count):
                res = requests.get(settings.BASEURL+'/Invoices?$orderby=DocEntry&$skip='+str(skip)+'', cookies=r.cookies, verify=False)
                ...
                for opt in opts['value']:
                    ...
                    if mycursor.rowcount != 1:
                        ...
                        for line in opt['DocumentLines']:
                            ...
                            line_sql = "INSERT INTO `Invoice_documentlines` (`LineNum`, `InvoiceID`, `Quantity`, `UnitPrice`, `DiscountPercent`, `ItemCode`, `TaxCode`, `ItemDescription`, `U_FGITEM`, `CostingCode2`, `ProjectCode`, `FreeText`) VALUES ('"+str(line['LineNum'])+"', '"+str(InvoiceID)+"', '"+str(line['Quantity'])+"', '"+str(line['UnitPrice'])+"', '"+str(lDiscountPercent)+"', '"+str(line['ItemCode'])+"', '"+str(line['TaxCode'])+"', '"+str(line['ItemDescription'])+"', '"+str(line['U_FGITEM'])+"', '"+str(line['CostingCode2'])+"', '"+str(line['ProjectCode'])+"', '"+str(line['FreeText'])+"');"
                            print(line_sql)
