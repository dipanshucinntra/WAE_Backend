...
from .models import *
from ServiceContract.serializers import *
...
#MasterServiceContract Create API
@api_view(['POST'])
def master_create(request):
    try:    
        res = MasterServiceContract_add(request)    
        if type(res) == int:
...
def MasterServiceContract_add(request):
        try:
            model = MasterServiceContract(**request.data)
            model.full_clean()
            model.save()
            req = MasterServiceContract.objects.latest('id')
            return req.id			
        except Exception as e:
            return str(e)
...
def ServiceContract_add(request):
        try:
            ...
            request.data['ServiceItem'] = json.dumps(request.data['ServiceItem'])            
            del request.data['id']
...
#MasterServiceContract All API
@api_view(["GET"])
def master_all(request):
    try:
        servicecontract_obj = MasterServiceContract.objects.all().order_by("-id")
...
#MasterServiceContract One API
@api_view(["POST"])
def master_one(request):
    try:
        id = request.data['id']
        if MasterServiceContract.objects.filter(pk=id).exists():
            Objs = MasterServiceContract.objects.filter(pk=id)
            #result = showServiceContract(Objs)
            result = MasterServiceContractSerializer(Objs, many=True)
            return Response({"message": "Success","status": 200,"data":result.data})
        else:
...
def master_update(request):
    id=request.data['id']
    fetchid=MasterServiceContract.objects.get(pk=id)
    
    if request.method=='PUT':
        req=MasterServiceContractSerializer(fetchid,data=request.data)
        if req.is_valid():
            req.save()
...
def update(request):
    ...
    request.data['ServiceContractsItem'] = json.dumps(request.data['ServiceContractsItem'])
    request.data['ServiceItem'] = json.dumps(request.data['ServiceItem'])
    request.data['CheckList'] = json.dumps(request.data['CheckList'])
...
#Master Contract Listing 
@api_view(["POST"])
def master_filter_key(request):
    try:
        res = GetMasterServiceContract(**request.data)
        return Response({"message": "Success","status": 200,"data":res})
    except Exception as e:
        return Response({"message": str(e),"status": 201,"data":[]})

def GetMasterServiceContract(**inp):
    print("--",inp['filter'])
    print("*--",*inp['fields'])
    
    if MasterServiceContract.objects.filter(**inp['filter']).exists():
        #{"filter":{"master_id":1, "ServiceContractOwner":120}, "fields":["id","master_id"]}
        Objs = MasterServiceContract.objects.filter(**inp['filter']).values(*inp['fields']).order_by("-id")
        ObjJson = MasterServiceContractSerializer(Objs, many=True)
        return ObjJson.data
...
#Project Listing 
@api_view(["POST"])
def servicecontract_filter_key(request):
    try:
        res = GetServiceContract(**request.data)
        return Response({"message": "Success","status": 200,"data":res})
    except Exception as e:
        return Response({"message": str(e),"status": 201,"data":[]})

def GetServiceContract(**inp):
    print("--",inp['filter'])
    print("*--",*inp['fields'])
    
    if ServiceContract.objects.filter(**inp['filter']).exists():
        #{"filter":{"master_id":1, "ServiceContractOwner":120}, "fields":["id","master_id"]}
        Objs = ServiceContract.objects.filter(**inp['filter']).values(*inp['fields']).order_by("-id")
        ObjJson = ServiceContractSerializer(Objs, many=True)
        return ObjJson.data
        #return Response({"message": "Success","status": 200,"data":ObjJson.data})
    else:      
        return []
