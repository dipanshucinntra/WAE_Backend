...
from Amendment.serializers import AmendmentSerializer
...
def create(request):
    # try:
        ...
        if SolutionType =="Lease" or SolutionType == "Sale":
            # if Order.objects.filter(PoNo=PoNo, CardCode=CardCode).exists():
            #     return Response({"message":"PO No. already exists with this Customer","status":201,"data":[]})
            if PoNo!="":
                order_data = Order.objects.filter(CardCode=CardCode).exclude(CancelStatus="csYes")
                print("fffeeeeeeeeeeeee")
                po_status = 0
                for obj in order_data:
                    print("eeeeeeeeeeeee")
                    data = obj.PoNo
                    re_data = re.findall(r'[0-9A-Za-z]', data)
                    val1 = ('').join(re_data)
                    input_data = re.findall(r'[0-9A-Za-z]', PoNo)
                    val2 = ('').join(input_data)
                    if val1 == val2:
                        po_status+=1
                        break
                if po_status >0:
        ...
        emp_objj = Employee.objects.filter(SalesEmployeeCode=qt.SalesPersonCode).first()
        send_notify = Notification(Title="Order created", Description="Click To Check", Type="Action", Read="0", SourceType="Order", SourceID=fetchid, Emp=emp_objj.reportingTo, SourceTime=time, CreatedDate=date, CreatedTime=time)    
        send_notify.save()
        if float(DiscountPercent)>7.0:
            apprvl_emp = Employee.objects.filter(role="ceo").first()
            send_notify = Notification(Title="Order created", Description="Click To Check", Type="Action", Read="0", SourceType="Order", SourceID=fetchid, Emp=apprvl_emp.SalesEmployeeCode, SourceTime=time, CreatedDate=date, CreatedTime=time)    
            send_notify.save()
        ...
        if settings.SAPORD == True:
            ...
            if emp_role =="salesman":
                if AppSlave.objects.filter(Level=3).exists():
                   slave_obj =  AppSlave.objects.filter(Level=3)
                   min = slave_obj[0].Min
                   max = slave_obj[0].Max
                else:
                    max = 7
            ...
            print("max",max)
            print("ds", float(DiscountPercent))
                    ...
                    if request.data['AddOnDocumentLines'] !=[]:
                        ...
                        for liness in addlines:
                            if line['ItemCode'] == liness['ParentItemCode']:
                                model_liness = AddOnDocumentLinesHistory(documentlineshistory_id= model_lines, OrderID = fetchid, LineNum=NewLine, Quantity = liness['Quantity'], UnitPrice = liness['UnitPrice'], ItemCode = liness['ItemCode'], ParentItemCode=liness['ParentItemCode'], ItemDescription = liness['ItemDescription'])
                                model_liness.save()
                                NewLine+=1
            return Response({"message":"successful","status":200, "data":[json.loads(json.dumps(request.data))]})
        else:
    ...
    try:
        ...
        for line in lines:
            if "id" in line:
                ...
                model_line.CostingCode2=line['CostingCode2']
                model_line.FreeText=line['FreeText']
                model_line.customize=line['customize']               
                model_line.save()
            else:
                lastline = DocumentLines.objects.filter(OrderID = fetchid).order_by('-LineNum')[:1]
                NewLine = int(lastline[0].LineNum) + 1
...
def OrderShow(Orders_obj):
    ...
    for qt in Orders_obj:         
        ...
        try:
            amendent = Amendment.objects.get(approval_status="Process", order_id=qt.id) 
            finalOrder['machine_sp_type'] = amendent.machine_sp_type  
            finalOrder['ops_revision'] = amendent.ops_revision
        except Exception as e:
             finalOrder['machine_sp_type'] = ""
        if qt.OrdLevel1:
            approval_id = qt.OrdLevel1
        if qt.OrdLevel2:
            approval_id = qt.OrdLevel2
        if qt.OrdLevel3:
            approval_id = qt.OrdLevel3
        if not approval_id:
            finalOrder["approval_id"] = ""
            finalOrder["approval_name"] = ""
        else:
            finalOrder["approval_id"] = approval_id.SalesEmployeeCode
            finalOrder["approval_name"] = approval_id.SalesEmployeeName
...
def OrderShowPage(Orders_obj):
    ...
    for qt in Orders_obj:
        ...
        order_obj["SalesPersonCode"] = qt.SalesPersonCode
        if qt.OrdLevel1:
            approval_id = qt.OrdLevel1
        if qt.OrdLevel2:
            approval_id = qt.OrdLevel2
        if qt.OrdLevel3:
            approval_id = qt.OrdLevel3
        if not approval_id:
            order_obj["approval_id"] = ""
            order_obj["approval_name"] = ""
        else:
            order_obj["approval_id"] = approval_id.SalesEmployeeCode
            order_obj["approval_name"] = approval_id.SalesEmployeeName
...
def approve(request):
    ...
    try:
        ...
        send_notify = Notification(Title="Order status", Description="Click To Check", Type="Action", Read="0", SourceType="Order", SourceID=qtid, Emp=order_emp, SourceTime=time, CreatedDate=date, CreatedTime=time)    
        send_notify.save()
...
#Order Payment Term Update
@api_view(['POST'])
def pay_update(request):
    fetchid = request.data['oid']
    try:
        Pay1 = float(request.data['U_Pay1'])
        Pay2 = float(request.data['U_Pay2'])
        Pay3 = float(request.data['U_Pay3'])
        Pay4 = float(request.data['U_Pay4'])
        Pay5 = float(request.data['U_Pay5'])
        
        pay = [Pay1,Pay2,Pay3,Pay4,Pay5]
        if sum(pay) != 100:
            return Response({"message":"Payment terms should be 100%","status":201,"data":[]})

        model = Order.objects.get(pk = fetchid)
        OrderStgs = OrderStage.objects.filter(OrderId=fetchid).order_by('Stageno')
        prj = 0
        if Project.objects.filter(order_id=fetchid).exists():        
            ProjectObj = Project.objects.filter(order_id=fetchid)[0]
            ProjectStgs = ProjectStage.objects.filter(ProjectId=ProjectObj.id, Name__icontains="Invoice").order_by('Stageno')
            prj=1
        
        if int(OrderStgs[0].Status) !=1:
            oo = OrderStgs[0]
            oo.PaymentPercentage = Pay1
            oo.save()            
            
            if prj==1:
                pp = ProjectStgs[0]
                pp.PaymentPercentage = Pay1        
                pp.save()            
            
            model.U_Pay1 = Pay1
        
        if int(OrderStgs[1].Status) !=1:
            oo = OrderStgs[1]
            oo.PaymentPercentage = Pay2
            oo.save()
            
            if prj==1:
                pp = ProjectStgs[1]
                pp.PaymentPercentage = Pay2
                pp.save()
            
            model.U_Pay2 = Pay2
        
        if int(OrderStgs[2].Status) !=1:
            oo = OrderStgs[2]
            oo.PaymentPercentage = Pay3
            oo.save()
            
            if prj==1:
                pp = ProjectStgs[2]
                pp.PaymentPercentage = Pay3
                pp.save()
            
            model.U_Pay3 = Pay3
        
        if int(OrderStgs[3].Status) !=1:
            oo = OrderStgs[3]
            oo.PaymentPercentage = Pay4
            oo.save()
            
            if prj==1:
                pp = ProjectStgs[3]
                pp.PaymentPercentage = Pay4
                pp.save()
            
            model.U_Pay4 = Pay4
        
        if int(OrderStgs[4].Status) !=1:
            oo = OrderStgs[4]
            oo.PaymentPercentage = Pay5
            oo.save()
            
            if prj==1:
                pp = ProjectStgs[4]
                pp.PaymentPercentage = Pay5
                pp.save()
...
#list service order
@api_view(["POST"])
def item_wise_services(request):
    try:
        ItemSerialNo = request.data['ItemSerialNo']
        serviceOrderObjs = AMCSalesOrder.objects.filter(ItemSerialNo = ItemSerialNo)
        finaldata = showServiceOrder(serviceOrderObjs)
        return Response({"message":"Successfull","status":201,"data":finaldata})
    except Exception as e:
        return Response({"message":str(e),"status":201,"data":[]})

#get all stage by order id    
@api_view(["POST"])    
def stage_all(request):
    oid = request.data['oid']
    stg = OrderStage.objects.filter(OrderId=oid).order_by('Stageno')
    print(stg)
    stg_json = OrderStageSerializer(stg, many=True)
...
def order_status(request):
    try:
        SalesEmployeeCode = request.data['SalesEmployeeCode']
        level = tree(SalesEmployeeCode)
        ord_status = request.data['ord_status'].lower()
        print(level)
        ord_obj = ""
        if ord_status =="rejected":
            if int(level) == 3:
                ord_obj = Order.objects.filter(OrdLevel3_id=SalesEmployeeCode,OrdLevel3Status="Rejected").order_by("-id")
            elif int(level) == 2:
                ord_obj = Order.objects.filter(OrdLevel2_id=SalesEmployeeCode,OrdLevel2Status="Rejected").order_by("-id")
            elif int(level) == 1:
                ord_obj = Order.objects.filter(OrdLevel1_id=SalesEmployeeCode,OrdLevel1Status="Rejected").order_by("-id")
            print(ord_obj)
        if ord_status == "approved":
            if int(level) == 3:
                ord_obj = Order.objects.filter(OrdLevel3_id=SalesEmployeeCode,OrdLevel3Status="Approved").order_by("-id")
            elif int(level) == 2:
                ord_obj = Order.objects.filter(OrdLevel2_id=SalesEmployeeCode,OrdLevel2Status="Approved").order_by("-id")
            elif int(level) == 1:
                ord_obj = Order.objects.filter(FinalStatus="Approved").order_by("-id")
        if ord_status == "pending":
            if int(level) == 3:
                ord_obj = Order.objects.filter(OrdLevel3_id=SalesEmployeeCode,OrdLevel3Status="Pending").order_by("-id")
            elif int(level) == 2:
                ord_obj = Order.objects.filter(OrdLevel2_id=SalesEmployeeCode,OrdLevel2Status="Pending").order_by("-id")
            elif int(level) == 1:
                ord_obj = Order.objects.filter(OrdLevel1_id=SalesEmployeeCode,OrdLevel1Status="Pending").order_by("-id")
            print(ord_obj)

        allqt = OrderShow(ord_obj)                        
        return Response({"message": "Success","status": 200,"data":allqt})
    except Exception as e:
...
#Order All By BP API
@api_view(["POST"])
def all_bybp(request):
    try: 
            quot_obj = Order.objects.filter(CardCode=request.data['CardCode']).order_by("-id")
            allqt = OrderShow(quot_obj)			
            return Response({"message": "Success","status": 200,"data":allqt})
    except Exception as e:
            return Response({"message": str(e), "status": 201, "data":[]})
...
#Delivery One API
@api_view(["GET"])
def bp_list_byorder(request):
    try:
        allbp = []
        bpList = Order.objects.values_list('CardCode', flat=True).distinct()
        print(bpList)
...
def sync(request):
    try:   
        res = settings.CALLAPI('get', '/Orders', 'api', '')        
        objs = json.loads(res.text)['value']
        print('objs')
        for obj in objs:        
            print(obj['DocEntry'])
            print(obj['DocDate'])
            print(obj)
            #PaymentTermsTypes(GroupNumber=obj['GroupNumber'], PaymentTermsGroupName=obj['PaymentTermsGroupName']).save()
            obj['Comments']=''
            obj['U_Pay1']=0
            obj['U_Pay2']=0
            obj['U_Pay3']=0
            obj['U_Pay4']=0
            obj['U_Pay5']=0
            serializer = OrderSerializer(data=obj)
            if serializer.is_valid(raise_exception=True):
                serializer.save()
                obj_id = Order.objects.latest('id').id
                print("order id ", obj_id)
                lines = obj['DocumentLines']
                if len(lines) > 0:
                    for line in lines:
                        line['TaxCode'] = ''
                        line['CostingCode2'] = ''
                        line['OrderID'] = obj_id
                        LineSer = DocumentLinesSerializer(data=line)
                        if LineSer.is_valid(raise_exception=True):
                            LineSer.save()
                            c1 = DocumentLines.objects.latest('id').id
                            print("itm", c1)
                
                line1 = obj['AddressExtension']
                if len(line1) > 0:
                    
                    line1['OrderID']=obj_id
                    line1['BillToBuilding']=''
                    line1['BillToCity']=''
                    line1['BillToZipCode']=''
                    line1['BillToState']=''
                    line1['BillToStreet']=''
                    ...
                    LineSer1 = AddressExtensionSerializer(data=line1)
                    if LineSer1.is_valid(raise_exception=True):
                        LineSer1.save()
                        c2 = AddressExtension.objects.latest('id').id
                        print("add", c2)
