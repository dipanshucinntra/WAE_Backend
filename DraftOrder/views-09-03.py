...
def create(request):
    try:
        ...
        try:
            ...
            PlumbingStatusSide = PlumbingStatusSide, CivilWorkSide = CivilWorkSide, SiteSurveySide = SiteSurveySide, SiteSurveySerialNo=SiteSurveySerialNo, CrainCharges = CrainCharges, LabourCharges = LabourCharges, ConsultingFee=ConsultingFee, SolutionType=SolutionType)
...
def OrderShow(Orders_obj):
    ...
    for qt in Orders_obj:
        ...
        print('----Attach----')
        try:
            if Attachment.objects.filter(LinkID = qt.id, LinkType="DraftOrder").exists():
                attachment_dls = Attachment.objects.filter(LinkID = qt.id, LinkType="DraftOrder")
