    ...
    try:
        ...
        try:
            ...
            for File in request.FILES.getlist('Attach'):
                ...
                att=Attachment(File=attachmentsImage_url, Caption=Caption, LinkType="DraftOrder", LinkID=fetchid, CreateDate=CreateDate, CreateTime=CreateTime, UpdateDate=UpdateDate, UpdateTime=UpdateTime, FileName = FileName)
                
                att.save()
            
        except Exception as e:
            print('Error in Draft')
            return Response({"message":str(e),"status":201,"data":[{"Error":"Error Draft Save"}]})
        try:
            ...
            print('Error Address Save')
            DraftOrder.objects.filter(pk=qt.id).delete()
            return Response({"message":str(e),"status":201,"data":[{"Error":"Error Address Save"}]})
        ...
        try:
            ...
            DraftOrder.objects.filter(pk=qt.id).delete()
            print('Error Docline Save')
            return Response({"message":str(e),"status":201,"data":[{"Error":"Error Docline Save"}]})
        
        if len(addon_lines) > 0:
            try:
                LineNum = 0
                for addon_line in addon_lines:
                    model_addlines = AddOnDocumentLines(LineNum = LineNum, OrderID = qt.id, Quantity = addon_line['Quantity'], UnitPrice = addon_line['UnitPrice'], ItemCode = addon_line['ItemCode'], ParentItemCode = addon_line['ParentItemCode'], ItemDescription = addon_line['ItemDescription'])
                    model_addlines.save()
                    print('AddOnDocline Save')
                    LineNum=LineNum+1
            except Exception as e:
                AddOnDocumentLines.objects.filter(OrderID=qt.id).delete()
                DraftOrder.objects.filter(pk=qt.id).delete()
                print('Error AddOnDocline Save')
                return Response({"message":str(e),"status":201,"data":[{"Error":"Error AddOnDocline Save"}]})
...
def OrderShow(Orders_obj):
    ...
    for qt in Orders_obj:
        ...
        order_obj = DraftOrderSerializer(qt)
        ...
        print('--DocumentLines--')
        print(finalOrder['DocumentLines'])
        ...
        try:
            ...
            print('----error Project----')
            print(str(e))
            return Response({"message": "Error in Project","status": 201,"data":[{"error":str(e)}]})        
        
        print('qt.AddOnDocumentLines')
        try:
            print('try')
        ...
        print('----PaymentGroupCode----')
        try:
            if qt.PaymentGroupCode != "" or qt.PaymentGroupCode != []:
        ...
        if qt.CardCode !="":
            ...
            if BusinessPartner.objects.filter(CardCode = qt.CardCode).exists():
                ...
                if CustCode.objects.filter(OrderId=qt.id).exists(): 
                    ...
                    OrdUrn = DraftOrder.objects.get(pk=qt.id)
                    OrdUrn.URN = BPURN
...
def all_filter(request):
    ...
    # if "U_OPPID" in json_data:
        # if json_data['U_OPPID'] !='':
            ...
            # quot_obj = DraftOrder.objects.filter(U_OPPID=json_data['U_OPPID']).order_by("-id")
            # if len(quot_obj) ==0:
        ...
        if json_data['SalesPersonCode']!="":
            ...
            for ke in json_data.keys():
                if ke =='U_FAV' :
                    print("yes filter")
                    if json_data['U_FAV'] !='':
                        quot_obj = DraftOrder.objects.filter(SalesPersonCode__in=SalesPersonID, U_FAV=json_data['U_FAV']).order_by("-id")
                        if len(quot_obj) ==0:
                            return Response({"message": "Not Available","status": 201,"data":[]})
                        else:
                            allqt = OrderShow(quot_obj)
                ...
                # elif ke =='U_TYPE' :
                    # if json_data['U_TYPE'] !='':
                        # quot_obj = DraftOrder.objects.filter(SalesPersonCode__in=SalesPersonID, U_TYPE=json_data['U_TYPE']).order_by("-id")
                        # if len(quot_obj) ==0:
                            # return Response({"message": "Not Available","status": 201,"data":[]})
                        # else:
                ...
                # elif ke =='Status' :
                    # if json_data['Status'] !='':
                        # quot_obj = DraftOrder.objects.filter(SalesPersonCode__in=SalesPersonID, Status=json_data['Status']).order_by("-id")
                        # if len(quot_obj) ==0:
                            # return Response({"message": "Not Available","status": 201,"data":[]})
                        # else:
                            # quot_json = OrderSerializer(quot_obj, many=True)
                    ...
                    print("no filter")
                    # qt = DraftOrder.objects.filter(SalesPersonCode__in=SalesPersonID).order_by("-id")
                    # quot_json = OrderSerializer(quot_obj, many=True)
                    # return Response({"message": "Success","status": 200,"data":quot_json.data})
                    quot_obj = DraftOrder.objects.filter(SalesPersonCode__in=SalesPersonID).order_by("-id")
                    allqt = OrderShow(quot_obj)
...
@api_view(["GET"])
def all(request):
    Orders_obj = DraftOrder.objects.all().order_by("-id")
...
def one(request):
    ...
    Orders_obj = DraftOrder.objects.filter(id=id)
    print('Orders_obj')
    print(Orders_obj)
...
def cancel(request):
    ...
    try:
        odr=DraftOrder.objects.get(DocEntry=fetchid)
...
def delete(request):
    fetchid=request.data['id']
    try:
        odr=DraftOrder.objects.filter(id=fetchid).first()
        if odr:
            odr.delete()                  
            return Response({"message":"successful","status":200,"data":[]})        
    except:
