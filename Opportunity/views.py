...
from global_fun import getAllReportingToIdsSubdep
from .models import *
...
def create(request):
    try:
        ...
        if len(json.loads(request.data['OppItem'])) != 0:
            ...
            for line in lines:
                try:
                    model_lines = OppItem(LineNum = LineNum, OppID = OppID, Quantity = line['Quantity'], UnitPrice = line['UnitPrice'], DiscountPercent = line['DiscountPercent'], ItemCode = line['ItemCode'], ItemDescription = line['ItemDescription'], TaxCode = line['TaxCode'], U_FGITEM = line['U_FGITEM'], CostingCode2 = line['CostingCode2'], ProjectCode = line['ProjectCode'], FreeText = line['FreeText'], Tax = line['Tax'], UomNo = line['UomNo'], IT_MICharges = line['IT_MICharges'], IT_LOCharges = line['IT_LOCharges'], IT_Intall = line['IT_Intall']) #added by millan on 03-November-2022
                    model_lines.save()
                    LineNum=LineNum+1
                except Exception as e:
            ...
            for line in lines:
                try:
                    ...
                    # if "id" in line:
                        ...
                        itmObj.IT_MICharges = line['IT_MICharges']
                        itmObj.IT_LOCharges = line['IT_LOCharges']
                        itmObj.IT_Intall = line['IT_Intall']
                        #added by millan on 03-November-2022
...
def all_filter_page(request):
    ...
    if "SalesPersonCode" in json_data:        
        if json_data['SalesPersonCode']!="":
            ...
            objs = Opportunity.objects.filter(Q(OpportunityName__icontains=SearchText, SalesPerson__in=SalesPersonID)|Q(CardCode__icontains=SearchText, SalesPerson__in=SalesPersonID)|Q(CustomerName__icontains=SearchText, SalesPerson__in=SalesPersonID)|Q(Remarks__icontains=SearchText, SalesPerson__in=SalesPersonID))    
            objs = settings.FILTER(json_data['field'], objs, "opportunity")    
            count = objs.count()
            objs = objs[page['startWith']:page['endWith']]            
            allqt = showOpportunity(objs)                
...
def excelDownload(request):
    ...
    try:
        oppIds = request.data['OppIds']
        oppObjs = Opportunity.objects.filter(id__in = oppIds).order_by('id')
        path = "leadDemo.xlsx"
        ...
        for obj in oppObjs:            
            leadId = obj.U_LEADID
            U_LSOURCE = obj.U_LSOURCE
            CurrentStageName = obj.CurrentStageName
            BPStatus = obj.BPStatus
            QTStatus = obj.QTStatus
            TDStatus = obj.TDStatus
            ODStatus = obj.ODStatus
            StartDate = obj.StartDate
            ClosingDate = obj.ClosingDate
            StatusRemarks = obj.StatusRemarks
            Opp_TYPE = obj.U_TYPE
            OppStatus = obj.OppStatus
            if OppStatus == "":
                if str(ODStatus) == "1":
                    CurrentStageName = "Order"
                elif str(TDStatus) == "1":
                    CurrentStageName = "Tender"
                elif str(QTStatus) == "1":
                    CurrentStageName = "Quotation"
                elif str(BPStatus) == "1":
                    CurrentStageName = "Business Partner"         
            else:
                CurrentStageName = OppStatus           

            strStartDate = datetime.datetime.strptime(StartDate, '%Y-%m-%d').strftime('%m/%d/%Y')
            strClosingDate = datetime.datetime.strptime(ClosingDate, '%Y-%m-%d').strftime('%m/%d/%Y')
            # datetime.datetime.strptime("2013-1-25", '%Y-%m-%d').strftime('%m/%d/%Y')
            arrDate = ClosingDate.split("-")

            leadTyp = ""
            leadStatus = "Other"
            leadSourch = ""
            if Lead.objects.filter(pk = leadId).exists():
                leadObj = Lead.objects.get(pk = leadId)
                # leadStatus = leadObj.status
                leadSourch =leadObj.source
                leadTyp = leadObj.tender
                if leadTyp == 1:
                    leadStatus = "Tender"

            customer_status = obj.OppType

            CardCode = obj.CardCode
            bObj = BusinessPartner.objects.get(CardCode = CardCode)
            bpName = bObj.CardName
            IndustryCode = bObj.Industry
            industryObj = Industries.objects.get(IndustryCode = IndustryCode)

            if BPAddresses.objects.filter(BPCode = CardCode).exists():
                bpObj = BPAddresses.objects.get(BPCode = CardCode)
                oppCity = bpObj.City
            else:
                oppCity = ""
            ...
            Quantity = ""
            UnitPrice = ""
            U_PROBLTY = obj.U_PROBLTY 
            if OppItem.objects.filter(OppID = obj.id).exists():
                itemObj = OppItem.objects.filter(OppID = obj.id)
                ItemDescription = itemObj[0].ItemDescription
                ItemCode = itemObj[0].ItemCode
                Quantity = itemObj[0].Quantity
                UnitPrice = itemObj[0].UnitPrice
            else:
                ItemDescription = ""
                ItemCode = ""
                Quantity = ""
                UnitPrice = ""
            wb_obj['FUNNEL'].cell(row = rowNo, column = 1, value = bObj.CardName).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 2, value = oppCity).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 3, value = Opp_TYPE).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 4, value = U_LSOURCE).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 5, value = leadStatus).font = fontStyle
            # wb_obj['FUNNEL'].cell(row = rowNo, column = 5, value = obj.OppType).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 6, value = obj.DivName).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 7, value = ItemDescription).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 8, value = ItemCode).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 9, value = Quantity).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 10, value = UnitPrice).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 11, value = U_PROBLTY ).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 13, value = CurrentStageName).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 14, value = strStartDate).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 15, value = strClosingDate).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 17, value = industryObj.IndustryName).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 18, value = obj.ContactPersonName).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 19, value = arrDate[1]).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 20, value = arrDate[0]).font = fontStyle
            wb_obj['FUNNEL'].cell(row = rowNo, column = 21, value = StatusRemarks).font = fontStyle            
            rowNo = rowNo+1
        fileName = uuid.uuid4()
        filepath = "/static/oppertunity-excel/"+str(fileName)+".xlsx"
        wb_obj.save("bridge"+filepath)
        return Response({"message":"successful","status":"200","data":[{'path': filepath}]})
        # return Response({"message":"successful","status":"200","data":oppJson.data})
    except Exception as e:
        return Response({"message":"Error","status":"201","data":[str(e)]})

#Opp status update
@api_view(['POST'])
def opp_status_update(request):
    try:
        fetchid = request.data['Opp_Id']
        OppStatus = request.data['OppStatus']
        if Opportunity.objects.filter(pk = fetchid).exists():
            Opportunity.objects.filter(pk = fetchid).update(OppStatus = OppStatus)
            return Response({"message":"successful","status":200,"data":[]})
        else:
            return Response({"message":"Invalid Opp_Id","status":201,"data":[]}) 
    except Exception as e:
        return Response({"message":str(e),"status":201,"data":[]})

#initation Create API
@api_view(['POST'])
def initiation_create(request):
    try:
        StartDate = request.data['StartDate']
        CompletedDate = request.data['CompletedDate']
        Comments = request.data['Comments']
        Opp_Id = request.data['Opp_Id']
        UpdateDate = request.data['UpdateDate']
        UpdateTime = request.data['UpdateTime']

        model=Initiation(StartDate=StartDate, CompletedDate=CompletedDate, Opp_Id=Opp_Id, UpdateDate=UpdateDate, UpdateTime=UpdateTime)
        
        model.save()
        qt = Initiation.objects.latest('id')
        fetchid = qt.id
        ...
        for File in request.FILES.getlist('Attach'):
            ...
            att=Attachment(File=attachmentsImage_url, Caption=Caption, LinkType="Initiation", LinkID=fetchid, CreateDate=UpdateDate, CreateTime=UpdateTime, UpdateDate=UpdateDate, UpdateTime=UpdateTime)            
            att.save()            
...
#Initiation One API
@api_view(["POST"])
def initiation_one(request):
    try:
        Opp_Id=request.data['Opp_Id']
        Initiation_obj = Initiation.objects.get(Opp_Id=Opp_Id)
        Initiation_json = InitiationSerializer(Initiation_obj)
        
        ObjJson = json.loads(json.dumps(Initiation_json.data))
        
        Attach = Attachment.objects.filter(LinkType="Initiation", LinkID=Opp_Id)
...
#Initiation Update API
@api_view(['POST'])
def initiation_update(request):
    try:
        fetchid = request.data['id']
        model = Initiation.objects.get(pk = fetchid)
        model.StartDate = request.data['StartDate']
        model.CompletedDate = request.data['CompletedDate']
        ...
        if int(request.data['Status']) ==1:
            Opportunity.objects.filter(id=request.data['Opp_Id']).update(INStatus=1)
...
#Initiation One API
@api_view(["POST"])
def presite_one(request):
    try:
        Opp_Id=request.data['Opp_Id']
        PreSite_obj = PreSite.objects.get(Opp_Id=Opp_Id)
        PreSite_json = PreSiteSerializer(PreSite_obj)
        
        ObjJson = json.loads(json.dumps(PreSite_json.data))
        
        Attach = Attachment.objects.filter(LinkType="PreSite", LinkID=Opp_Id)
        AttachJson = AttachmentSerializer(Attach, many=True)        
        ObjJson['Attach'] = AttachJson.data               
...
#PreSite Update API
@api_view(['POST'])
def presite_update(request):
    try:
        fetchid = request.data['id']
        model = PreSite.objects.get(pk = fetchid)
        model.StartDate = request.data['StartDate']
        model.CompletedDate = request.data['CompletedDate']
        model.Comments = request.data['Comments']
        
        model.Shaft = request.data['Shaft']
        model.ShaftWidth = request.data['ShaftWidth']
        model.ShaftDepth = request.data['ShaftDepth']
        model.TotalTravel = request.data['TotalTravel']
        model.SideWallA = request.data['SideWallA']
        model.SideWallC = request.data['SideWallC']
        model.PitDepth = request.data['PitDepth']
        ...
        model.save()
        if int(request.data['Status']) ==1:
            Opportunity.objects.filter(id=request.data['Opp_Id']).update(PRStatus=1)
        return Response({"message":"successful","status":"200","data":[]})
    except Exception as e:
        return Response({"message":str(e),"status":"201","data":[]})
...
#Site One API
@api_view(["POST"])
def site_one(request):
    try:
        Opp_Id=request.data['Opp_Id']
        Site_obj = Site.objects.get(Opp_Id=Opp_Id)
        Site_json = SiteSerializer(Site_obj)
        
        ObjJson = json.loads(json.dumps(Site_json.data))
        
        Attach = Attachment.objects.filter(LinkType="Site", LinkID=Opp_Id)
        AttachJson = AttachmentSerializer(Attach, many=True)        
        ObjJson['Attach'] = AttachJson.data               
...
def site_update(request):
    try:
        fetchid = request.data['id']
        model = Site.objects.get(pk = fetchid)
        model.StartDate = request.data['StartDate']
        model.CompletedDate = request.data['CompletedDate']
        model.Comments = request.data['Comments']
        ...
        model.save()
        if int(request.data['Status']) ==1:
            Opportunity.objects.filter(id=request.data['Opp_Id']).update(STStatus=1)
        return Response({"message":"successful","status":"200","data":[]})
    except Exception as e:
        return Response({"message":str(e),"status":"201","data":[]})
...
def rfq_one(request):
    try:
        Opp_Id=request.data['Opp_Id']
        RFQ_obj = RFQ.objects.get(Opp_Id=Opp_Id)
        RFQ_json = RFQSerializer(RFQ_obj)
        
        ObjJson = json.loads(json.dumps(RFQ_json.data))
        
        Attach = Attachment.objects.filter(LinkType="RFQ", LinkID=Opp_Id)
        AttachJson = AttachmentSerializer(Attach, many=True)        
        ObjJson['Attach'] = AttachJson.data               
...
#RFQ Update API
@api_view(['POST'])
def rfq_update(request):
    try:
        fetchid = request.data['id']
        model = RFQ.objects.get(pk = fetchid)
        model.StartDate = request.data['StartDate']
        model.CompletedDate = request.data['CompletedDate']
        model.Comments = request.data['Comments']
        model.Status = request.data['Status']
        model.SupplierCost = request.data['SupplierCost']
        model.UpdateDate = request.data['UpdateDate']
        model.UpdateTime = request.data['UpdateTime']
        model.save()
        if int(request.data['Status']) ==1:
            Opportunity.objects.filter(id=request.data['Opp_Id']).update(RFStatus=1)
...
def showOpportunity(objs):
    allopp = []
    for obj in objs:
        opp_json = OpportunitySerializer(obj, many=False)
        finalObjs = json.loads(json.dumps(opp_json.data))

        allLines = []
        lines = OppItem.objects.filter(OppID = obj.id)
        allLines = OppItemSerializer(lines, many=True)
        finalObjs['OppItem'] = allLines.data
        allopp.append(finalObjs)
    return allopp    