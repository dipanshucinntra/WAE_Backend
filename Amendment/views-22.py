...
def create(request):
    ...
    amd = Amendment(order_id=order_id,ops_revision=ops_revision,client_name=client_name,ops_number=ops_number,amendment=amendment,reason=reason,open_date=open_date,close_date=close_date,created_by=created_by)
    amd.save()
...
def update(request):
    ...
    close_date = request.data["close_date"]
    created_by = request.data["created_by"]
    Amendment.objects.filter(id=id).update(order_id=order_id,ops_revision=ops_revision,client_name=client_name,ops_number=ops_number,amendment=amendment,reason=reason,open_date=open_date,close_date=close_date,created_by=created_by)
    return Response({"message":"successful","status":"200","data":[]})
