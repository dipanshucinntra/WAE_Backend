...
currentTime = datetime.today().strftime("%H:%M %p")
...
def create(request):
    try:
        ...
        report_objj = emp_obj[0].reportingTo
        send_notify = Notification(Title="Quotation created", Description="Click To Check", Type="Action", Read="0", SourceType="Quotation", SourceID=fetchid, Emp=report_objj, SourceTime=time_time, CreatedDate=currentDate, CreatedTime=time_time)    
        send_notify.save()
        if float(DiscountPercent)>7.0:
            apprvl_emp = Employee.objects.filter(role="ceo").first()
            send_notify = Notification(Title="Quotation created", Description="Click To Check", Type="Action", Read="0", SourceType="Quotation", SourceID=fetchid, Emp=apprvl_emp.SalesEmployeeCode, SourceTime=time_time, CreatedDate=currentDate, CreatedTime=time_time)    
            send_notify.save()
...
def approve(request):
    ...
    try:
        ...
        send_notify = Notification(Title="Quotation status", Description="Click To Check", Type="Action", Read="0", SourceType="Quotation", SourceID=qtid, Emp=quot_emp, SourceTime=time_time, CreatedDate=currentDate, CreatedTime=time_time)    
        send_notify.save()
    ...
    try:
        ...
        for line in lines:
            ...
            if line['id'] != "":
                ...
                model_line.IncidentalCharges=line['IT_MICharges']
                model_line.CivilWorkCharges=line['IT_LOCharges']
                model_line.PlumbingCharges=line['IT_Intall']
...
def rejected(request):
    try:
        ...
        print(level)
        if int(level) == 3:
            quot_obj = Quotation.objects.filter(Level3_id=SalesEmployeeCode,Level3Status="Rejected").order_by("-id")
        elif int(level) == 2:
            quot_obj = Quotation.objects.filter(Level2_id=SalesEmployeeCode,Level2Status="Rejected").order_by("-id")
        elif int(level) == 1:
            quot_obj = Quotation.objects.filter(Level1_id=SalesEmployeeCode,Level1Status="Rejected").order_by("-id")
        print(quot_obj)
...
def all_filter_page(request):
    ...
    if "SalesPersonCode" in json_data:
        ...
        if json_data['SalesPersonCode']!="":
            ...
            objs = Quotation.objects.filter(Q(U_QUOTNM__icontains=SearchText, SalesPersonCode__in=SalesPersonID)|Q(CardCode__icontains=SearchText, SalesPersonCode__in=SalesPersonID)|Q(CardName__icontains=SearchText, SalesPersonCode__in=SalesPersonID)|Q(FinalStatus__icontains=SearchText, SalesPersonCode__in=SalesPersonID))
            
            objs = settings.FILTER(json_data['field'], objs, "quotation")    
            count = objs.count()
            objs = objs[page['startWith']:page['endWith']]
            
            allqt = QuotationShow(objs)
...
def remarksHistory(request):
    try:
        qtid = request.data['id']
        allRemarks = []
        if QuotStatusRemarks.objects.filter(QuotationID = qtid).exists():
            remarkObj = QuotStatusRemarks.objects.filter(QuotationID = qtid).order_by('-id')
            print(remarkObj)
            # remarkJson = QuotStatusRemarksSerializer(remarkObj, many=True)
            for obj in remarkObj:
                SalesEmployeeCode = obj.SalesEmployeeCode
                remarkJson = QuotStatusRemarksSerializer(obj)
...
@api_view(['POST'])
def pi_attachments_upload(request):
    try:
        # print(request.data)
        fileIds = []
        QuotationID = request.data['QuotationID']
        if Quotation.objects.filter(pk = QuotationID).exists():
            attechmentsImage_url = "" 
            if len(request.FILES.getlist('File')) != 0:
                # print("in attch")
                for Attachment in request.FILES.getlist('File'):
                    # print('in loop')
                    target ='./bridge/static/image/PI-Attachment'
                    os.makedirs(target, exist_ok=True)
                    fss = FileSystemStorage()
                    ...
                    QuotAttachments( QuotationID = QuotationID, File = attechmentsImage_url).save()
                    attchId = QuotAttachments.objects.latest('-id')
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
@api_view(['POST'])
def pi_attachments_delete(request):
    try:
        AttchementIds = request.data['AttchementIds']
        if len(AttchementIds) != 0:
            for id in AttchementIds:
                print('in loop', id)
                if QuotAttachments.objects.filter(pk = id).exists():
                    fileObj = QuotAttachments.objects.get(pk = id)
                    # fileUrl = "http://103.234.187.197:8012"+fileObj.File
                    # fileUrl = "/home/www/b2b/massead_pre/bridge/bridge"+fileObj.File
                    # fileUrl = "./bridge"+fileObj.File
