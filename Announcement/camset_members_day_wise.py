...
for campaign in allRow:
    ...
    if leadFlag:
        ...
        for contactPrson in allLead:
            userData = {
                ...
                'CountryCode': contactPrson[3],
                'Email': contactPrson[4]
            }
    ...
    for user in userList:
        ...
        if Phone != "" and Email != "":
            sqlSelectCamSet = "SELECT * FROM `Campaign_campaignsetmembers` WHERE `CampSetId_id` = '"+str(camSetId)+"' AND `Phone` = "+ str(Phone) +"  AND `CountryCode` = '"+str(CountryCode)+"' AND `Email` = '"+str(Email)+"'"
            # print(sqlSelectCamSet)
            mycursor.execute(sqlSelectCamSet)
            allRow = mycursor.fetchall()
            
            if not allRow:
                insertCamSetMember ="INSERT INTO `Campaign_campaignsetmembers`(`Name`, `Phone`,`CountryCode`, `Email`, `CampSetId_id`) VALUES ('"+str(Name)+"', '"+str(Phone)+"', '"+str(CountryCode)+"', '"+str(Email)+"', '"+str(camSetId)+"')"
