...
def create_campset(request):
    try:
        ...
        if 1 > 2:
            ...
            request.data['AllEmployeeList'] = request.data['AllEmployee']
...
def showCamSet(objs):
    ...
    for obj in objs:
        ...
        if CampaignSetOwnerId.SalesEmployeeCode != "":
            sobj = Employee.objects.filter(SalesEmployeeCode = CampaignSetOwnerId.SalesEmployeeCode).values("SalesEmployeeCode","SalesEmployeeName")
            sobj_json = EmployeeSerializer(sobj, many=True)
            finalCamSet['CampaignSetOwner'] = sobj_json.data
        else:
            finalCamSet['CampaignSetOwner'] = []
        
        if CreateById.SalesEmployeeCode != "":
            CreateByobj = Employee.objects.filter(SalesEmployeeCode = CreateById.SalesEmployeeCode).values("SalesEmployeeCode","SalesEmployeeName")
...
def showCamSetOne(objs):
    ...
    for obj in objs:
        ...
        role_arr = obj.Role.strip('[').strip(']').replace(' ', '').split(",")
        if len(obj.Role) !=0:
        ...
        emp_arr = obj.AllEmployee.strip('[').strip(']').replace(' ', '').split(",")
        print('emp_arr')
        print(emp_arr)
        if len(obj.AllEmployee) !=0:
            emp = Employee.objects.filter(SalesEmployeeCode__in = emp_arr).values("id", "SalesEmployeeCode", "SalesEmployeeName")
            print(emp)
            emp_json = EmployeeSerializer(emp, many=True)
            finalCamSet['AllEmployee'] = emp_json.data
        else:
            finalCamSet['AllEmployee'] = []
            
        sub_arr = obj.EmpSubDep.strip('[').strip(']').replace(' ', '').split(",")
        ...
        dep_arr = obj.EmpDep.strip('[').strip(']').replace(' ', '').split(",")
        if len(obj.EmpDep) !=0:
...
def update_campset(request):
    try:
        ...
        request.data['EmpDep'] = str(request.data['EmpDep'])        
        request.data['EmpSubDep'] = str(request.data['EmpSubDep'])        
        request.data['Role'] = str(request.data['Role'])        
        request.data['AllEmployeeList'] = request.data['AllEmployee']
        request.data['AllEmployee'] = str(request.data['AllEmployee']).replace("'","").replace('"','').replace(' ','')
        ...
        return Response({"status":"201","message":"unsuccess","data":[fetchJson.errors]})
    except Exception as e:
...
def ListCampaignSetMember(requestData, CamSetId):
    ...
    if leadFlag:
        sqlSelectLead = "SELECT `id`, `contactPerson`, `phoneNumber`,`country_code`, `email`, `leadType`, `status` FROM `Lead_lead` " + str(leadwhere)
        print(sqlSelectLead)
        ...
        for contactPrson in leadObj:
            userData = {
                ...
                'CountryCode': contactPrson.country_code,
    ...
    if AllEMPCheck == 1:
        ...
        empFlag = True
    elif requestData['AllEmployeeList'] =="":
        empwhere = " where Active='tYES' "
        empFlag = True  
    else:
        empwhere = " where Active='tYES' "
        AllEmployee = ",".join(map(str, requestData['AllEmployeeList']))
        empwhere += ' and `SalesEmployeeCode` in('+ AllEmployee +')'
        empFlag = True
    if empFlag:
        sqlSelectEmp = "SELECT `id`, `firstName`, `Mobile`,`CountryCode`, `Email`, `role`, `dep_id`, `subdep_id`, `Active` FROM `Employee_employee` " + str(empwhere)
        print(sqlSelectEmp)
